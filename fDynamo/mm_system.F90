...
!===============================================================================
   ...
   ! . Check for an error.
   IF ( IOSTAT /= 0 ) CALL PRINT_ERROR ( "MM_SYSTEM_CONSTRUCT", "I/O Error.", IOSTAT )
   ...
   IF ( QMISSING ) CALL PRINT_ERROR ( "FILL_ATOMS_AND_MM_TERMS", "Missing energy function parameters." )
   ...
   !============================================================================
      ...
      DO I = 1,NANGL
         ...
         ! . No match.
         QMISSING = .TRUE.
         WRITE ( PRINT_LINE, "(A,3(2X,A))" ) "Missing angle parameter for:", TYPES(T1)%NAME, TYPES(T2)%NAME, TYPES(T3)%NAME
      ...
      DO I = 1,NBND
         ...
         ! . No match.
         QMISSING = .TRUE.
         WRITE ( PRINT_LINE, "(A,2(2X,A))" ) "Missing bond parameter for:", TYPES(T1)%NAME, TYPES(T2)%NAME
         CALL PRINT_PARAGRAPH
      ...
      DO I = 1,NDIHE
         ...
         IF ( MM1234 == 0 ) THEN
            ...
            ELSE
               MM = 0
               QMISSING = .TRUE.
               WRITE ( PRINT_LINE, "(A,4(2X,A))" ) "Missing dihedral parameter for:", &
...
! - expresion de los dihedros (phase)
      ...
      DO I = 1,NIMPR
         ...
         ELSE
            MM = 0
            QMISSING = .TRUE.
            WRITE ( PRINT_LINE, "(A,4(2X,A))" ) "Missing improper parameter for:", &
                   TYPES(T1)%NAME, TYPES(T2)%NAME, TYPES(T3)%NAME, TYPES(T4)%NAME
	    CALL PRINT_PARAGRAPH
         END IF
   ...
   DO I = 1,NRESID
      ...
      IF ( NBND > 0 ) CALL NAMES_TO_INDICES ( TOTATOM, 2, NBND, RESINF%BONDS, RESINF%NAMES, BNDATM )
      IF ( NIMP > 0 ) CALL NAMES_TO_INDICES ( TOTATOM, 4, NIMP, RESINF%IMPRS, RESINF%NAMES, IMPATM )
   ...
   END DO
!write ( output, "(A,3i6)" ) "AT END OF LOOP, TOTATOM, TOTBOND, TOTIMPR = ", TOTATOM, TOTBOND, TOTIMPR
!write ( output, "(a)" ) "lnkhgh:"
!write ( output, * ) lnkhgh
   ...
   IF ( POS_NEW > 0 ) THEN
      CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "The last residue cannot have a `+R' bond specification." )
   END IF
...
!      CHARACTER ( LEN = ATOM_NAME_LENGTH ), DIMENSION(:,:), INTENT(IN)  :: NAME_IN
      ...
      DO I = 1,N
         DO J = 1,M
            ...
            IF ( TMP == 0 ) THEN
               CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "Unknown atom name: "//NAME_IN(J,I)//"." )
            ELSE
               INDEX_OUT(J,I) = TMP
            END IF
      ...
      RESINF%NBONDS = NBND
      RESINF%NIMPRS = NIMP
!write ( 6, "(A,3I10)" ) "ALL> NATM, NBND, NIMP = ", NATM, NBND, NIMP
      ! . Initialize the pointers.
      ...
      IF ( NATM > 0 ) ALLOCATE ( RESINF%CHRGS(1:NATM), RESINF%NAMES(1:NATM), RESINF%TYPES(1:NATM) )
      IF ( NBND > 0 ) ALLOCATE ( RESINF%BONDS(1:2,1:NBND), RESINF%BINDX(1:NBND) )
      IF ( NIMP > 0 ) ALLOCATE ( RESINF%IMPRS(1:4,1:NIMP), RESINF%IINDX(1:NIMP) )
...
!      INTEGER, DIMENSION(:),   INTENT(IN)    :: BINDX
      INTEGER, DIMENSION(:),   POINTER       :: BINDX
      ...
      DO I = 1,NBNDTMP
         ...
         ELSE
            ...
            IF ( B2 <= 0 ) CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "A bond contains no non-link atoms." )
...
!write ( output, "(A)" ) "LNKHGH:"
!write ( output, * ) LNKHGH
!write ( output, "(A)" ) "LNKLOW:"
!write ( output, * ) LNKLOW
!write ( output, "(A)" ) "LNKNUM:"
!write ( output, * ) LNKNUM
!write ( output, "(A)" ) "BINDX"
!write ( output, * ) BINDX
!WRITE ( output, "(A,6i6)" ) "I, BINDX, CURRES, B1, B2, MAXVAL = ", I, BINDX(I), CURRES, B1, B2, MAXVAL ( LNKNUM(1:2,BINDX(I)) )
               ! . The current residue has the highest index in the link.
               IF ( CURRES == MAXVAL ( LNKNUM(1:2,BINDX(I)) ) ) THEN
!WRITE ( output, "(A,i6)" ) "LNKLOW(BINDX(I)) = ", LNKLOW(BINDX(I))
                  ! . Store the bond.
                  ...
                  ELSE
                     CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "There is a missing `*R' specification in a link." )
                  END IF
            ...
            CASE ( CODE_MINUS )
               ...
               ! . Check ATM_NEG.
               IF ( ATM_NEG > 0 ) THEN
                  CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "Only one connection to the previous residue is allowed." )
               ELSE
                  ATM_NEG = B2
               END IF
               ...
               ELSE
                  CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "Invalid bond to a previous residue." )
               END IF
            ...
            CASE ( CODE_PLUS )
               ...
               ! . Check ATM_POS.
               IF ( ATM_POS > 0 ) THEN
                  CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "Only one connection to the next residue is allowed." )
               ELSE
                  ATM_POS = B2
               END IF
            ...
            ! . Unknown index.
            CASE DEFAULT
               CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "Invalid bond atom index." )
            END SELECT
         END IF
      ...
      NATM = MIN ( RESINF_IN%NATOMS, RESINF_OUT%NATOMS )
      ...
      ! . Copy the arrays.
      IF ( RESINF%NATOMS > 0 ) THEN
         RESINF%CHRGS(1:RESINF%NATOMS) = RESIDUE%CHARGES(1:RESINF%NATOMS)
         RESINF%NAMES(1:RESINF%NATOMS) = RESIDUE%NAMES(1:RESINF%NATOMS)
         RESINF%TYPES(1:RESINF%NATOMS) = RESIDUE%TYPES(1:RESINF%NATOMS)
      END IF
...
!      INTEGER, DIMENSION(:),   INTENT(IN)    :: IINDX, LNKLOW
      INTEGER, DIMENSION(:),   POINTER       :: IINDX
      INTEGER, DIMENSION(:),   INTENT(IN)    :: LNKLOW
      INTEGER, DIMENSION(:,:), INTENT(IN)    :: LNKNUM
         ...
         DO J = 1,4
            ...
            CASE ( CODE_MINUS )
               ...
               IF ( POS_OLD > 0 ) THEN
                  IMPATM(J,I) = POS_OLD
               ELSE
                  CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "Improper specified to previous residue without bond to `-R'." )
               END IF
      ...
      ! . Local scalars.
      INTEGER :: I, INDEX, J
!write ( output, "(a)" ) "TMPIMP:"
!write ( output, "(4i6)" ) tmpimp
!write ( output, "(a)" ) "lnkhgh:"
!write ( output, * ) lnkhgh
      ! . Loop over the impropers.
      DO I = START,STOP
         ...
         DO J = 1,4
            ...
            IF ( TMPIMP(J,I) == 0 ) THEN
               ...
               ELSE
                  CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "Improper specified without corresponding `+R' bond." )
               END IF
...
!write ( 6, "(A,3I10)" ) "INI> NATM, NBND, NIMP = ", RESINF%NATOMS, RESINF%NBONDS, RESINF%NIMPRS
      IF ( RESINF%NATOMS > 0 ) DEALLOCATE ( RESINF%CHRGS, RESINF%NAMES, RESINF%TYPES )
      IF ( RESINF%NBONDS > 0 ) DEALLOCATE ( RESINF%BONDS, RESINF%BINDX )
      IF ( RESINF%NIMPRS > 0 ) DEALLOCATE ( RESINF%IMPRS, RESINF%IINDX )
      ...
      IF ( VARIANT%NDELETES > 0 ) THEN
         ...
         DO I = 1,VARIANT%NDELETES
            ...
            ! . Unknown atom.
            IF ( .NOT. QFOUND ) CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "Unknown atom to delete: " // &
      ...
      IF ( VARIANT%NCHARGES > 0 ) THEN
         ...
         DO I = 1,VARIANT%NCHARGES
            ...
            ! . Type name not found.
            CALL PRINT_ERROR ( "GENERATE_STRUCTURE", "Unknown atom to change charge: " // &
   ...
   IF ( PRESENT ( FILE ) ) THEN
      ...
      ! . Check for an error.
      IF ( IOSTAT /= 0 ) CALL PRINT_ERROR ( "READ_SEQUENCE_FILE", "I/O Error.", IOSTAT )
   ...
   IF ( UNIT == INPUT ) THEN
      CALL PRINT_PARAGRAPH ( TEXT = "Sequence data read from input stream." )
   ELSE
      CALL PRINT_PARAGRAPH ( TEXT = "Sequence data read from " // FILE )
   END IF
