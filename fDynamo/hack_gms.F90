...
PRIVATE
PUBLIC :: GAMESS_SETUP, GAMESS_ENERGY, GAMESS_GRADIENT, GAMESS_HESSIAN

CONTAINS


SUBROUTINE GAMESS_NON_BONDED( SELECTION )
    IMPLICIT NONE
...
SUBROUTINE GAMESS_REMOVE_MM_TERMS( SELECTION )
    IMPLICIT NONE
...
SUBROUTINE GAMESS_SETUP( SELECTION )
    IMPLICIT NONE
    LOGICAL, DIMENSION(1:NATOMS), INTENT(IN) :: SELECTION
    ...
    END DO
    DEALLOCATE( IDX )
    CALL GAMESS_REMOVE_MM_TERMS( SELECTION )
    QUSEBAQM = .FALSE.
END SUBROUTINE
...
SUBROUTINE GAMESS_ENERGY( ENERGY )
    IMPLICIT NONE
    ...
    REAL( KIND=DP ), DIMENSION(:), ALLOCATABLE :: EXT_CORD, EXT_ATMN, EXT_GRAD, EXT_HESS

    ALLOCATE( SELE(1:NATOMS) )
    CALL GAMESS_NON_BONDED( SELE )
...
!            END IF
    ...
    CODE = 0
    CALL GAMESS_SERVER( CODE, NT, NATOMSQM, EXT_ATMN, EXT_CORD, EXT_ENER, EXT_GRAD, EXT_HESS )
...
SUBROUTINE GAMESS_GRADIENT( ENERGY, GRADIENT )
    IMPLICIT NONE
...
SUBROUTINE GAMESS_HESSIAN( ENERGY, GRADIENT, HESSIAN )
    IMPLICIT NONE
...
!    REAL ( KIND = DP ), DIMENSION(1:(3*NFREE*(3*NFREE+1))/2), INTENT(INOUT) :: HESSIAN
    ...
    ALLOCATE( EXT_CORD(1:3*NT), EXT_ATMN(1:NT), EXT_GRAD(1:3*NT), EXT_HESS(1:NH) )
...
!            END IF
    ...
    DEALLOCATE( SELE, EXT_ATMN, EXT_CORD, EXT_GRAD, EXT_HESS )
END SUBROUTINE
