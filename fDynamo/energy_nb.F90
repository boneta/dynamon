...
!===============================================================================
   ...
   !---------------------------------------------------------------------------------
   SUBROUTINE ENERGY_NON_BONDING_CALCULATE ( EELECT, ELJ, VIRIAL, GRADIENT, HESSIAN, QMLJ )
   !---------------------------------------------------------------------------------
   ...
   REAL ( KIND = DP ), INTENT(INOUT), OPTIONAL :: QMLJ
   ...
   REAL ( KIND = DP ) :: TMP_QMLJ, TMP_QMLJ14
   ...
   IF ( NNBLISTMM > 0 ) THEN
      ...
      ! . Calculate the interactions.
      IF( .NOT. PRESENT( QMLJ ) ) THEN
      CALL INTERACTIONS ( EELECT, ELJ, VIRIAL, NNBLISTMM, NBLISTMM_FIRST, ATMCHG, ATMEPS, ATMSIG, GRADIENT, &
                          HESSIAN )
      ELSE
      CALL INTERACTIONS ( EELECT, ELJ, VIRIAL, NNBLISTMM, NBLISTMM_FIRST, ATMCHG, ATMEPS, ATMSIG, GRADIENT, &
                          HESSIAN, TMP_QMLJ )
   ...
   IF( PRESENT( QMLJ ) ) QMLJ = TMP_QMLJ14 + TMP_QMLJ
...
!-------------------------------------------------------------------------------
   ...
   !----------------------------------------------------------------------------------------------------------------
   SUBROUTINE INTERACTIONS ( EELECT, ELJ, VIRIAL, NRECORDS, NBLIST_FIRST, CHARGE, EDEPTH, SIGMA, GRADIENT, HESSIAN, QMLJ )
   !----------------------------------------------------------------------------------------------------------------
   ...
   REAL ( KIND = DP ), INTENT(OUT), OPTIONAL :: QMLJ
   REAL ( KIND = DP )                        :: TMP_QMLJ, TMP_INT
   ...
   DO IREC = 1,NRECORDS
      ...
      DO IINT = 1,SIZE(NBCURRENT%INTERACTIONS)
         ...
         ! . Avoid QM - QM interactions (Electrostatic or LJ)
         IF ( ATMQMI ( I ) > 0 .AND. ATMQMI ( J ) > 0 ) CYCLE

         ! . Skip fixed atoms auto-interactions...
         ...
         IF ( RIJ2 <= R2ON ) THEN
            ...
            ! - Skip electrostatic QM-MM interactions, either ATMCHG(QM) is null
            IF (.NOT. SKIP_ABINITIO) THEN
               IF( ( ATMQMI(I) == 0 .AND. ATMQMI(J) == 0 ) .OR. SKIP_ABINITIO ) &
            ...
            TMP_INT = EIJ * ( ( S12 - SHIFT_LJ12 ) - ( S6 - SHIFT_LJ6 ) )
            ELJ = ELJ + TMP_INT
            IF( ATMQMI(I) > 0 .OR. ATMQMI(J) > 0 ) TMP_QMLJ = TMP_QMLJ + TMP_INT
            ...
            IF (.NOT. SKIP_ABINITIO) THEN
               ! - split each contribution in order to apply for a QM(ab initio)/MM scheme
               DFele  = - ETEMP / RIJ2
               DFvdw  = 6.0_DP * EIJ * ( S6 - 2.0_DP * S12 ) / RIJ2
               DF = DFele + DFvdw
            ELSE
         ...
         ELSE
            ...
            ! . Calculate the Lennard-Jones interaction.
            TMP_INT = EIJ * ( K12 * ( S6 - SHIFT_LJ12 )**2 - K6 * ( S3 - SHIFT_LJ6 )**2 )
            ELJ = ELJ + TMP_INT
            IF( ATMQMI(I) > 0 .OR. ATMQMI(J) > 0 ) TMP_QMLJ = TMP_QMLJ + TMP_INT
            ...
            IF (.NOT. SKIP_ABINITIO) THEN
            ! - split each contribution in order to apply for a QM(ab initio)/MM scheme
               DFele = - QIJ * ( A / R3 + B * S + 3.0_DP * C * R + 5.0_DP * D * R3 ) 
               DFvdw = - 6.0_DP * EIJ * ( 2.0_DP * K12 * S6 * ( S6 - SHIFT_LJ12 ) - K6 * S3 * ( S3 - SHIFT_LJ6 ) ) / RIJ2
               DF = DFele + DFvdw
            ELSE
         ...
         IF (.NOT. SKIP_ABINITIO) THEN
            ! . Split each contribution in order to apply for a QM(ab initio)/MM scheme
            ! . Lennard Jonnes is always incorporated (it's classical...)
            GRADIENT(1:3,I) = GRADIENT(1:3,I) + DFvdw * DR
            GRADIENT(1:3,J) = GRADIENT(1:3,J) - DFvdw * DR
            ! . Electrostatical contribution is only calculated if its a MM atom
            ! (interacting with a QM atom with a fitted charge)
            IF( ATMQMI( I ) == 0 ) GRADIENT(1:3,I) = GRADIENT(1:3,I) + DFele * DR
            IF( ATMQMI( J ) == 0 ) GRADIENT(1:3,J) = GRADIENT(1:3,J) - DFele * DR
         ELSE
         ...
         IF (.NOT. SKIP_ABINITIO) THEN
            ! In a QM(ab initio)/MM calculation, VDW QM-MM is full, and Elec QM-MM is
            ! partial only for the MM atoms (QM already polarized); thus
            ! only QM atoms should be allowed to be in the hessian matrix. MM atoms
            ! interacting with the fitted QM charges will lose the electrostatic hessian terms.
            DF = DFvdw
            IF ( RIJ2 <= R2ON ) THEN
               D2F = 24.0_DP * EIJ * ( 7.0_DP * S12 - 2.0_DP * S6 ) / ( RIJ2 * RIJ2 )
            ELSE
               D2F = ( - 6.0_DP * EIJ * ( 2.0_DP * K12 * S6 * ( 7.0_DP * SHIFT_LJ12 - 13.0_DP * S6 ) - &
...
! ---
! Una manera de parchear este problema (hessiana solo QM), es incorporar de modo
! numérico la contribución non-bonded, ya que los gradientes sí que estan bien.
! Habría que hacer una subrutina, que calcule todos los términos de hessiana de
! modo analítico, excepto los NB, que tendría que hacer un 3N del gradiente...
! ---
